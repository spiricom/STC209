/*! p5.sound.js v0.2.16 2016-01-17 */
...
env = function () {
  ...
  p5.Env = function (t1, l1, t2, l2, t3, l3, t4, l4) {
    ...
    // set to true for exponential instead of linear response
    this.exp = false;
...
p5.Env.prototype.setADSR = function (t1, t2, l2, t3) {
    this.aTime = t1;
    this.dTime = t2 || 0;
    this.sLevel = l2 || 0;
    this.rTime = t3 || 0;
  ...
  p5.Env.prototype.setExp = function (expOrLin) {
    //report it to the log for now so that we can tell that the value was really changed
    console.log('exponential curve = ' + expOrLin);
    this.exp = expOrLin;
  };
  ...
  p5.Env.prototype.play = function (unit, secondsFromNow) {
    ...
    if (this.exp == true) 
    {
      ...
      this.control.exponentialRampToValueAtTime(currentVal, t);
    }
    else
    {
    ...
    if (this.exp == true) {
      // attack
      //console.log("ramping to " + this.aLevel + " in this time " + this.aTime);

      this.control.exponentialRampToValueAtTime(this.aLevel, t + this.aTime);
      // decay to decay level

      //console.log("ramping to " + this.dLevel + " in this time " + (this.aTime + this.dTime));
      this.control.exponentialRampToValueAtTime(this.dLevel, t + this.aTime + this.dTime);
      // hold sustain level
      this.control.exponentialRampToValueAtTime(this.sLevel, t + this.aTime + this.dTime + this.sTime);
      // release
      this.control.exponentialRampToValueAtTime(this.rLevel, t + this.aTime + this.dTime + this.sTime + this.rTime);
    } else {
      // attack
...
p5.Env.prototype.ramp = function (unit, destination, secondsFromNow, fadeLength) {
    if (unit) {
      if (this.connection !== unit) {
        this.connect(unit);
      }
    }
    this.control.setTargetAtTime(destination, p5sound.audiocontext.currentTime+secondsFromNow, fadeLength);
  };

p5.Env.prototype.ad = function (unit, destination, startTime, attackT, decayT) {
    if (unit) {
      if (this.connection !== unit) {
        this.connect(unit);
      }
    }
    this.control.setTargetAtTime(destination, p5sound.audiocontext.currentTime+startTime, attackT);
    this.control.setTargetAtTime(0.0, p5sound.audiocontext.currentTime+startTime+attackT, decayT);
  };

  p5.Env.prototype.adsrTrig = function (unit, destination, startTime) {
    if (unit) {
      if (this.connection !== unit) {
        this.connect(unit);
      }
    ...
    //console.log("hi");
    var attackT = (this.aTime * .2);
    var decayT = (this.dTime * .2);
    var susLevel = this.sLevel || 0;
    this.control.setTargetAtTime(destination, p5sound.audiocontext.currentTime+startTime, attackT);
    this.control.setTargetAtTime((susLevel * destination * .63), p5sound.audiocontext.currentTime+startTime+attackT, decayT);
  };

  p5.Env.prototype.adsrRel = function (unit, destination, startTime) {
    if (unit) {
      if (this.connection !== unit) {
        this.connect(unit);
      }
    ...
    this.control.cancelScheduledValues(p5sound.audiocontext.currentTime+startTime);
    var releaseT = (this.rTime * .2);
    this.control.setTargetAtTime(destination, p5sound.audiocontext.currentTime+startTime, releaseT);
  };
  /**
  ...
  p5.Env.prototype.triggerRelease = function (unit, secondsFromNow) {
    ...
    if (t - this.lastAttack < this.aTime) {
      ...
      if (this.exp == true) {
        this.control.exponentialRampToValueAtTime(this.aLevel, t + a);
        this.control.exponentialRampToValueAtTime(this.dLevel, t + a + this.dTime);
        this.control.exponentialRampToValueAtTime(this.sLevel, t + a + this.dTime + this.sTime);
        this.control.exponentialRampToValueAtTime(this.rLevel, t + a + this.dTime + this.sTime + this.rTime);
      } else {
      ...
      if (this.exp == true) {
        this.control.exponentialRampToValueAtTime(this.dLevel, t + d);
        // this.control.linearRampToValueAtTime(this.sLevel, t + d + this.sTime);
        this.control.exponentialRampToValueAtTime(this.sLevel, t + d + 0.01);
        this.control.exponentialRampToValueAtTime(this.rLevel, t + d + 0.01 + this.rTime);
      } else {
      ...
      if (this.exp == true) {
        this.control.exponentialRampToValueAtTime(this.sLevel, t + s);
        this.control.exponentialRampToValueAtTime(this.rLevel, t + s + this.rTime);
      } else {
      ...
      if (this.exp == true) {
        this.control.exponentialRampToValueAtTime(this.sLevel, t);
        this.control.exponentialRampToValueAtTime(this.rLevel, t + this.rTime);
      } else {
