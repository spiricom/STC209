...
var maxAttack = .005; // in seconds ... setting to .001 makes things very percussive, setting to > 1 makes them sound far away
...
function setup() 
{
  ...
  for (var i = 0; i < numNotes; i++)
  {
    ...
    for(var j = 0; j < numOsc; j++)
    {
      ...
      envelope[i][j] = new p5.Env(0,0,0,0);
      ...
      envelope[i][j].setADSR(random(.001, myMaxAttack), random(.01, maxDecay), 0.00,0.1 ); // turning sustain level to 0. makes an AD envelope
      osc[i][j].amp(0.);
...
function makeSoundAttack(time, playbackRate) 
{
  ...
  {
    // the 1.3 gain compensates for the 30% gain reduction from using the adsrTrig function (it never reaches 100%)
    envelope[whichNote][i].adsrTrig(osc[whichNote][i], (oscVols[whichNote][i] * random(.9, 1.0) * 1.3), time);
  }
